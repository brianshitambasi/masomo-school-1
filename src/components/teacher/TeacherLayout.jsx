...
const TeacherLayout = () => {
  ...
  const navItems = [
    { path: '/teacher-dashboard', icon: 'bi-grid', label: 'Dashboard' },
    { path: '/teacher-dashboard/classes', icon: 'bi-building', label: 'My Classes' },
    { path: '/teacher-dashboard/students', icon: 'bi-people-fill', label: 'Students' },
    { path: '/teacher-dashboard/assignments', icon: 'bi-journal-bookmark-fill', label: 'Assignments' },
    { path: '/teacher-dashboard/attendance', icon: 'bi-calendar-check-fill', label: 'Attendance' },
    { path: '/teacher-dashboard/grades', icon: 'bi-graph-up', label: 'Grades' },
    { path: '/teacher-dashboard/profile', icon: 'bi-person-fill', label: 'Profile' },
    { path: '/teacher-dashboard/settings', icon: 'bi-gear', label: 'Settings' },
  ]
    ...
    <div className="d-flex" style={{ minHeight: '100vh', background: '#f4f6f9' }}>
      ...
      <div style={{ position: isMobile ? 'fixed' : 'fixed', top: 0, left: 0, height: '100vh', zIndex: 1000, width: sidebarOpen ? '250px' : '0px', overflow: 'hidden', transition: 'width 0.3s ease', boxShadow: '2px 0 10px rgba(0,0,0,0.1)', backgroundColor: '#1a1a2e', display: 'flex', flexDirection: 'column' }}>
        <div className="p-3 text-center border-bottom border-secondary">
          ...
          <h5 className="text-white mb-0">{user?.name || 'Teacher'}</h5>
          <small className="text-white-50">Teacher Portal</small>
        </div>
        <nav className="flex-grow-1 p-3 overflow-auto">
          <ul className="nav nav-pills flex-column">
            {navItems.map((item) => (
              <li className="nav-item mb-2" key={item.path}>
                <NavLink to={item.path} end className={({ isActive }) => isActive ? 'nav-link bg-success text-light fw-bold rounded-3' : 'nav-link text-light rounded-3'}>
      ...
      <div className="flex-grow-1" style={{ marginLeft: isMobile ? 0 : (sidebarOpen ? '250px' : '0px'), transition: 'margin-left 0.3s ease', minHeight: '100vh', display: 'flex', flexDirection: 'column', width: '100%' }}>
        <nav className="navbar navbar-expand-lg bg-white shadow-sm" style={{ padding: '0 24px', borderBottom: '1px solid #e9ecef', height: '64px', width: '100%', position: 'sticky', top: 0, zIndex: 999, background: 'white' }}>
          <div className="container-fluid px-0">
            <div className="d-flex align-items-center">
              ...
              <Link to="/teacher-dashboard" className="navbar-brand fw-bold"><span className="text-success">Masomo</span> <span className="text-secondary">Teacher</span></Link>
            </div>
            <div className="d-flex align-items-center">
              <div className="dropdown">
                <a className="dropdown-toggle d-flex align-items-center text-decoration-none" href="#" role="button" data-bs-toggle="dropdown">
                  <div className="rounded-circle bg-success d-flex align-items-center justify-content-center me-2" style={{ width: '34px', height: '34px', fontSize: '13px', fontWeight: 'bold', color: 'white' }}>{user?.name?.charAt(0)?.toUpperCase() || 'T'}</div>
                  <div className="d-none d-md-block"><div className="fw-bold small" style={{ fontSize: '13px' }}>{user?.name || 'Teacher'}</div><small className="text-muted" style={{ fontSize: '10px' }}><i className="bi bi-shield-check me-1 text-success"></i>Teacher</small></div>
                </a>
        ...
        <main className="p-4" style={{ flex: 1, overflowY: 'auto', backgroundColor: '#f4f6f9', paddingTop: '20px', width: '100%' }}><Outlet /></main>
        <footer className="text-center py-3" style={{ backgroundColor: 'white', borderTop: '1px solid #e9ecef', color: '#6c757d', fontSize: '13px', width: '100%' }}>
          <div className="container-fluid"><span><i className="bi bi-mortarboard-fill text-success me-2"></i>Masomo School Management System © {new Date().getFullYear()}<span className="mx-2">|</span>Teacher Portal v1.0.0</span></div>
        </footer>
      </div>
    </div>
  )
